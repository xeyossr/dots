...
from concurrent.futures import ThreadPoolExecutor, as_completed  # <-- eklendi
...
class animecix:
    ...
    def fetch_anime_episodes(self, selected_id):
        ...
        def fetch_episodes_for_season(season_index):
            ...
            if data and 'videos' in data:
                for item in data['videos']:
                    ...
                    if name not in seen_episode_names:
                        ...
                        season_episodes.append({'name': name, 'url': episode_url, 'season_num': season_num})
            return season_episodes

        with ThreadPoolExecutor(max_workers=5) as executor:
            futures = [executor.submit(fetch_episodes_for_season, i) for i in seasons]
            for future in as_completed(futures):
                try:
                    for ep in future.result():
                        if ep["name"] not in seen_episode_names:
                            episodes.append(ep)
                            seen_episode_names.add(ep["name"])
                except Exception:
                    pass
        return episodes
...
class openanime:
    ...
    def get_episodes(self, slug):
        ...
        def fetch_episode_list(season):
            url = f"{self.base_url}/anime/{slug}/season/{season}"
            data = self.get_json(url)
            episodes = []
            if data:
                eps = data.get("season", {}).get("episodes", [])
        ...
        with ThreadPoolExecutor(max_workers=5) as executor:
            futures = [executor.submit(fetch_episode_list, s) for s in range(1, season_count + 1)]
            for future in as_completed(futures):
                try:
