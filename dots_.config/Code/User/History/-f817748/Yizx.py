...
def parse_last_output():
    ...
    for line in lines:
        ...
        RE_BOOT_LINE = re.compile(
            r"(?P<type>reboot)\s+system boot\s+(?P<kernel>[\w\.\-\*]+)?\s+(?P<start>\w{3}\s+\w{3}\s+\d+\s+\d{2}:\d{2})"
            r"(?:\s+-\s+(?P<end>[\w\s:]+))?(?:\s+\((?P<duration>[^)]+)\))?"
        )
...
if __name__ == "__main__":
    ...
    for entry in entries:
        ...
        if exit_type == "still running":
            shutdown = Text("still running", style="green")
            td = now - entry["start_dt"]
            uptime = format_timedelta(td)
            exit_label = "running"
        
        elif exit_type == "crash":
            if duration.strip():
                if "+" in duration:
                ...
                shutdown_str = shutdown_dt.strftime("%b %d, %Y %H:%M") if use_long_date else shutdown_dt.strftime("%Y-%m-%d %H:%M")
                shutdown = Text(shutdown_str)
                uptime = format_duration(duration)
            else:
                shutdown = Text("crashed?", style="bold red")
                uptime = "unknown"
            exit_label = "crash"

        if not duration.strip():
            uptime = format_timedelta(now - entry["start_dt"])
            shutdown = "[green]still running[/green]"
        else:
            if "+" in duration:
                days_part, time_part = duration.split("+")
                days = int(days_part.strip())
        ...
        if shutdown == "[green]still running[/green]":
            shutdown_text = Text("still running", style="green")
