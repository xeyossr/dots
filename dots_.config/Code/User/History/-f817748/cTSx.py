...
def parse_last_output():
    """Parse 'last -x' output to extract system boot entries."""
...
if __name__ == "__main__":
    ...
    table.add_column("Shutdown", style="bright_magenta", no_wrap=True, justify="left")
    ...
    for entry in entries:
        ...
        if use_long_date:
            boot_time = entry["start_dt"].strftime("%b %d, %Y %H:%M")
        ...
        exit_styles = {
            "crash": "bold red",
            "shutdown": "bright_green",
            "running": "yellow"
        }
        exit_text.stylize(exit_styles.get(exit_type, "white"))
