...
def fetch_list(token, user_id, media_type="ANIME"):
    url = "https://graphql.anilist.co"
    query = '''
    ...
    if not user or not user.get("mediaList"):
        show_error("Medya listesi bulunamadı veya boş.")
        return []

    for entry in user["mediaList"]:
...
def main():
    ...
    anime_list = fetch_list(token, user_id, "ANIME")
    manga_list = fetch_list(token, user_id, "MANGA")
