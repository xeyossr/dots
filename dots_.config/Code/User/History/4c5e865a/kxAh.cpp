...
string color_wrap(const string& text, const string& color_code, int width, bool left_align = true) {
    int visible_len = (int)text.length(); // Burada basit uzunluk kullandım. Eğer ANSI varsa bu biraz daha karmaşık olur ama metin zaten temiz, sadece renk kodları sarıyor
    string padded;
    if (left_align) {
        padded = text + string(width - visible_len, ' ');
    } else {
        padded = string(width - visible_len, ' ') + text;
    }
