...
def start_discord_rpc(client_id="1383421771159572600", filepath="/tmp/anime_details"):
    ...
    try:
        ...
        rpc = None
        rpc_initialized = False
        log_error("/tmp/anitr_error", e)
        send_notification("anitr-cli", f"Discord RPC başlatılamadı. Hata detayları: /tmp/anitr_error", "critical")
        return
...
def AnimeciX():    
    ...
    def update_watch_api(index, selected_id):
        ...
        try:
            ...
            log_error("/tmp/anitr_error", e)
            send_notification("anitr-cli", f"anitr-cli bir hatayla karşılaştı. Hata detayları: /tmp/anitr_error", "critical")
