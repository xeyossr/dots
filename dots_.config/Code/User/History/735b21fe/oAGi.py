...
rpc_timestamps = os.getenv("DISCORD_RPC_TIMESTAMPS", "Enabled")
...
def log_anime_details(details: str, state: str, large_text: str, source: str, source_url: str, path: str = "/tmp/anime_details"):
    data = {
        ...
        "state": state,
        "timestamps": (rpc_timestamps == "Enabled"),
...
def set_discord_rpc(client_id, filepath):
    # RPC bağlantısını başlat
    ...
    activity = {
        ...
        "activity": {
            ...
            "assets": {
                "large_image": data.get("large_image"),  # Discord geliştirici panelindeki görsel key
    ...
    # Etkinlik bilgisini Discord'a gönder
    rpc.send_data(1, {
        "cmd": "SET_ACTIVITY",
        "args": activity,
        "nonce": str(time.time())
    ...
    print("📺 Discord RPC etkinliği başarıyla ayarlandı.")

    # Bir süre etkinliği göster ve sonra RPC'yi temizle
    time.sleep(15)
    rpc.clear_activity()
    rpc.close()
    ...
    while True:
        ...
        if selected_option == "İzle" or selected_option == "Filmi izle":
            ...
            log_anime_details(
                ...
                state=f"{selected_episode_index + 1}/{total_episodes}" if not is_movie else f"{selected_anime_name}",
                large_text="",
                source=source,
