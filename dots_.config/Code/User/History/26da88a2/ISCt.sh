...
CUSTOM_DIR="$DOTFILES_DIR/dots_custom"
...
install_dotfiles() {
    ...
    # Copy custom dotfiles back
    if [ -d "$CUSTOM_DIR" ]; then
        for file in aliases gitconfig; do
            if [ -f "$CUSTOM_DIR/.$file" ]; then
                cp "$CUSTOM_DIR/.$file" "$HOME/.$file"
                echo "✅ Restored $file to ~/"
            else
                echo "⚠️ $CUSTOM_DIR/.$file not found, skipping."
            fi
        done
    else
        echo "⚠️ $CUSTOM_DIR not found, skipping custom dotfiles restore."
    fi
}
...
backup_dotfiles() {
    ...
    rm -rf "$CONFIG_DIR" "$LOCAL_DIR" "$ETC_DIR" "$USR_DIR" "$PKGS_DIR" "$CUSTOM_DIR"
    ...
    for file in \
        ...
        copy_with_path "$HOME/.config/$file" "$CONFIG_DIR"
    done

    echo "📂 Copying from ~/.local..."
    for bin in hyde-ipc hyde-shell hydectl; do
        copy_with_path "$HOME/.local/bin/$bin" "$LOCAL_DIR"
    done

    copy_with_path "$HOME/.local/lib/hyde" "$LOCAL_DIR"

    for share in fastfetch waybar hyde icons kio kxmlgui5; do
        copy_with_path "$HOME/.local/share/$share" "$LOCAL_DIR"
    done

    for state in hyde dolphinstaterc .staterc; do
        copy_with_path "$HOME/.local/state/$state" "$LOCAL_DIR"
    done

    echo "📂 Copying from /etc (sudo)..."
    copy_with_path "/etc/plymouth" "$ETC_DIR"

    echo "📂 Copying from /usr (sudo)..."
    copy_with_path "/usr/share/plymouth/themes/spinner" "$USR_DIR"

    echo "📂 Copying custom dotfiles (~/.aliases, ~/.gitconfig)..."
    for file in aliases gitconfig; do
        if [ -f "$HOME/.$file" ]; then
            cp "$HOME/.$file" "$CUSTOM_DIR/.$file"
            echo "✅ Copied: $HOME/.$file → $CUSTOM_DIR/.$file"
        else
