...
def select_menu(type: str, choices: list[str], msg: str, header, tui_menu_with_search_input: bool = True) -> str:
    if type.lower() == "tui":
        if tui_menu_with_search_input == True:
            return tui.get_selection_from_list(header, choices, msg, "fuzzy")
        elif tui_menu_with_search_input == False:
            return tui.get_selection_from_list(header, choices, msg, "list")
    elif type.lower() == "rofi":
