...
def mpv_play(url):
    # mpv'yi cache ile başlat
    subprocess.run([
        "mpv", "--cache=yes", "--cache-secs=60", "--no-video", "--quiet", 
        "--input-ipc-server=/tmp/mpvsocket", url
    ])
...
def ara_ve_çal(aranan):
    ...
    if video_id:
        ...
        # Şarkı çaldıktan sonra mpvc komutunu kullanarak kontrol paneliyle etkileşim
        subprocess.run(["mpvc", "--input-ipc-server=/tmp/mpvsocket", "status"])
