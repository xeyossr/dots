...
# YTMusic API'si ile müzik arama
ytmusic = YTMusic()
...
def ara_ve_çal(aranan):
    ...
    if video_id:
        ...
        print(f"Şarkı indiriliyor ve cmus ile çalınıyor...")

        # YouTube'dan mp3 dosyasını indir
        subprocess.run(["yt-dlp", "-x", "--audio-format", "mp3", url])

        # İndirilen şarkıyı cmus ile çal
        subprocess.run(["cmus", "-C", "clear"])  # cmus'i sıfırla
        subprocess.run(["cmus", "-C", "add", "*.mp3"])  # İndirilen mp3 dosyasını ekle
        subprocess.run(["cmus", "-C", "play"])  # Şarkıyı çalmaya başla
