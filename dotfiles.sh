    ...
    if [ -d "$HOME_DIR" ]; then
        for file in aliases gitconfig; do
            if [ -f "$HOME_DIR/.$file" ]; then
                ...
                echo "⚠️ $HOME_DIR/.$file not found, skipping."
            fi
        done
    else
        echo "⚠️ $HOME_DIR not found, skipping custom dotfiles restore."
    fi
}
...
backup_dotfiles() {
    ...
    echo "📂 Copying from /etc..."
    copy_with_path "/etc/plymouth" "$ETC_DIR"

    echo "📂 Copying from /usr..."
    ...
    if [ -d "$HOME/.icons" ]; then
        cp -r "$HOME/.icons" "$HOME_DIR/.icons"
        echo "✅ Copied: $HOME/.icons → $HOME_DIR/.icons"
    else
        echo "⚠️ Skipped (not found): $HOME/.icons"
    fi
